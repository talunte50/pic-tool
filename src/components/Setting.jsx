...
export default observer(() => {
    ...
    // 从 compressorState 反读当前值（分享链接恢复后，Setting 打开时同步展示真实参数）
    const o = compressorState.option;
    useEffect(() => {
        const qPercent = typeof o.jpeg === 'object' ? (o.jpeg?.quality ?? 0.75) * 100 : (Number(o.jpeg) || 0.75) * 100;
        setQuality(Math.round(qPercent));
        setFormat(o.format.target || 'auto');
        setColor(o.format.transparentFill || defaultOption.color);
        const mt = o.resize.method;
        setSizeType(mt || 'auto');
        setSize(mt ? (o.resize.width || o.resize.height) : undefined);
    }, []);
    ...
    const onSizeTypeChange = (value) => {
        setSizeType(value);
        setSize(defaultOption.size);
        compressorState.setSizeType(value, undefined);
    }
