...
description: "Learn how pic-tool Photo Background Remover uses a pure web workflow and the Xenova/modnet model to create clean background cutouts in your browser."
date: 2026-07-01
...
pic-tool loads the background-removal model inside the web page and runs the processing from the browser environment. The image is decoded by the browser, passed through the model, and converted into a foreground mask.
...
pic-tool uses the `Xenova/modnet` model from Hugging Face. The model is published for image segmentation and background removal workflows, and its model page shows usage through Transformers.js with a `background-removal` pipeline.
